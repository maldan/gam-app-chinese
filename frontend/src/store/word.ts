  ...
  actions: {
    ...
    async add(action: WordActionContext): Promise<void> {
      await Axios.post(`${action.rootState.main.API_URL}/word`, {
        ...
        transcription: action.rootState.modal.data.transcription.toLowerCase(),
        category:
        ...
        translate: {
          ...
          adjective:
            action.rootState.modal.data.translate?.adjective
              ...
              ?.filter(Boolean) || [],
          adverb:
            action.rootState.modal.data.translate?.adverb
              ?.split(',')
              ?.map((x: string) => x.trim())
              ?.filter(Boolean) || [],
          particle:
            action.rootState.modal.data.translate?.particle
              ?.split(',')
              ?.map((x: string) => x.trim())
              ?.filter(Boolean) || [],
        },
